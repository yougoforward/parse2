...
class gnn_loss_noatt(nn.Module):
    ...
    def forward(self, preds, targets):
        ...
        for i in range(len(preds[1])-1):
            ...
            loss_hb.append(self.criterion(pred_hb, targets[1]))
        loss_hb = sum(loss_hb)
        #half seg loss final
        pred1 = F.interpolate(input=preds[1][-1], size=(h, w), mode='bilinear', align_corners=True)
        pred = F.softmax(input=pred1, dim=1)
        #lovasz loss
        ...
        for i in range(len(preds[2])):
            ...
            #                           only_present=self.only_present))
            loss_fb.append(self.criterion(pred_fb, targets[2]))
        loss_fb = sum(loss_fb)
        #full seg loss final
        pred2 = F.interpolate(input=preds[2][-1], size=(h, w), mode='bilinear', align_corners=True)
        pred = F.softmax(input=pred2, dim=1)
        #lovasz loss
        ...
        return (loss_final+0.4*loss_final_hb+0.4*loss_final_fb + loss + 0.4 * loss_hb + 0.4 * loss_fb)/len(preds[1]) + 0.4 * loss_dsn
...
class gnn_loss_dp3(nn.Module):
    ...
    def forward(self, preds, targets):
        ...
        for i in range(len(preds[1])):
            pred_hb = F.interpolate(input=preds[1][i], size=(h, w), mode='bilinear', align_corners=True)
            loss_ce = self.criterion2(pred_hb, targets[1].long())
            pred_hb = F.softmax(input=pred_hb, dim=1)
            loss_hb.append(loss_ce + lovasz_softmax_flat(*flatten_probas(pred_hb, targets[1], self.ignore_index),
                                      only_present=self.only_present))
        # loss_hb = sum(loss_hb)
        ...
        for i in range(len(preds[2])):
            ...
            pred_fb = F.softmax(input=pred_fb, dim=1)
            loss_fb.append(loss_ce + lovasz_softmax_flat(*flatten_probas(pred_fb, targets[2], self.ignore_index),
                                      only_present=self.only_present))
        # loss_fb = sum(loss_fb)
        ...
        return (0.4*(loss[0] + loss_hb[0] + loss_fb[0])+(loss[-1] + 0.4*loss_hb[-1] + 0.4*loss_fb[-1])) + 0.4 * loss_dsn
class gnn_loss_dp32(nn.Module):
    """Lovasz loss for Alpha process"""

    def __init__(self, adj_matrix, ignore_index=None, only_present=True, upper_part_list=[1, 2, 3, 4], lower_part_list=[5, 6], cls_p=7, cls_h=3, cls_f=2):
        super(gnn_loss_dp32, self).__init__()
        self.edge_index = torch.nonzero(adj_matrix)
        self.edge_index_num = self.edge_index.shape[0]
        self.part_list_list = [[] for i in range(cls_p - 1)]
        for i in range(self.edge_index_num):
...
class gnn_loss_dp4(nn.Module):
    """Lovasz loss for Alpha process"""

    def __init__(self, adj_matrix, ignore_index=None, only_present=True, upper_part_list=[1, 2, 3, 4], lower_part_list=[5, 6], cls_p=7, cls_h=3, cls_f=2):
        super(gnn_loss_dp4, self).__init__()
        self.edge_index = torch.nonzero(adj_matrix)
        self.edge_index_num = self.edge_index.shape[0]
        self.part_list_list = [[] for i in range(cls_p - 1)]
        for i in range(self.edge_index_num):
    ...
    def forward(self, preds, targets):
        ...
        # return (sum(loss) + 0.4*sum(loss_hb) + 0.4*sum(loss_fb))/(len(preds[0]))+ 0.4 * loss_dsn + 0.1*(loss_fh_att + loss_up_att + loss_lp_att + loss_dp_att + loss_com_att)/(len(preds[3]))
        # return (loss[0] + 0.4*loss_hb[0] + 0.4*loss_fb[0]+loss[-1] + 0.4*loss_hb[-1] + 0.4*loss_fb[-1])+ 0.4 * loss_dsn + 0.2*(loss_fh_att + loss_up_att + loss_lp_att + loss_dp_att + loss_com_att)
        return 0.5*((loss[0] + 0.4*loss_hb[0] + 0.4*loss_fb[0]+loss[-1] + 0.4*loss_hb[-1] + 0.4*loss_fb[-1])+ 0.4 * loss_dsn)
