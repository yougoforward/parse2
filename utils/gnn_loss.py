...
class gnn_loss_dp3(nn.Module):
    ...
    def forward(self, preds, targets):
        ...
        for i in range(len(preds[0])):
            ...
            loss_ce = self.criterion(pred, targets[0])
            pred = F.softmax(input=pred, dim=1)
            loss.append(0.2*loss_ce + lovasz_softmax_flat(*flatten_probas(pred, targets[0], self.ignore_index), only_present=self.only_present))
        ...
        for i in range(len(preds[1])):
            ...
            pred_hb = F.softmax(input=pred_hb, dim=1)
            loss_hb.append(0.2*loss_ce + lovasz_softmax_flat(*flatten_probas(pred_hb, targets[1], self.ignore_index),
                                      only_present=self.only_present))
        # loss_hb = sum(loss_hb)
        ...
        for i in range(len(preds[2])):
            ...
            pred_fb = F.softmax(input=pred_fb, dim=1)
            loss_fb.append(0.2*loss_ce + lovasz_softmax_flat(*flatten_probas(pred_fb, targets[2], self.ignore_index),
                                      only_present=self.only_present))
        # loss_fb = sum(loss_fb)
